...
class Obraz:
    ...
    def save_modified_image_with_bboxes(self, output_path, bounding_boxes, skip_boxes=[]):
        ...
        modified_with_bboxes = np.array(self.modified_image)

        # Create a mask for the bounding boxes
        mask = np.zeros_like(modified_with_bboxes, dtype=np.uint8)
        ...
        darkened_background = np.where(mask == 0, modified_with_bboxes * 0.2, modified_with_bboxes)

        # Draw bounding boxes on the darkened background
        for bbox in bounding_boxes:
            x, y, w, h = bbox
            darkened_background[y:y+2, x:x+w, :] = [0, 255, 0]  # Top border
            darkened_background[y+h-2:y+h, x:x+w, :] = [0, 255, 0]  # Bottom border
            darkened_background[y:y+h, x:x+2, :] = [0, 255, 0]  # Left border
            darkened_background[y:y+h, x+w-2:x+w, :] = [0, 255, 0]  # Right border

        # Save the modified image with bounding boxes and darkened background using cv2
        cv2.imwrite(output_path, darkened_background)

    def save_modified_image(self, output_path, bounding_boxes, skip_boxes=[]):
        # Clone the original image to avoid modifying it directly
        modified_with_bboxes = np.array(self.original_image)
            ...
            if binary_image[row, col] == 255:
                ...
                while stack:
                    ...
                    for neighbor in neighbors:
                        ...
                        if 0 <= neighbor_pixel[0] < rows and 0 <= neighbor_pixel[1] < cols and binary_image[
                            neighbor_pixel] == 255:
